...
export class FlowTaskSettingTab extends PluginSettingTab {
  ...
  override display(): void {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "FlowTask For Super Productivity" });
    ...
    new Setting(containerEl).setName("Base URL").setDesc("Normalmente http://127.0.0.1:3876").addText((text) => text.setPlaceholder("http://127.0.0.1:3876").setValue(this.plugin.settings.baseUrl).onChange(async (value) => { this.plugin.settings.baseUrl = value.trim().replace(/\/$/, "") || "http://127.0.0.1:3876"; await this.plugin.saveSettings(); }));
    ...
    new Setting(containerEl).setName("Refresh interval").setDesc("Between 2 and 30 seconds.").addSlider((slider) => slider.setLimits(2, 30, 1).setValue(this.plugin.settings.pollingSeconds).setDynamicTooltip().onChange(async (value) => { this.plugin.settings.pollingSeconds = value; await this.plugin.saveSettings(); this.plugin.restartPolling(); }));
